...
const Notifications: React.FC<NotificationsProps> = ({ notifications, onClearAll, onAcceptConnectRequest, onDeclineConnectRequest, onViewProfile, onViewActivity }) => {
    return (
        <div className="container mx-auto px-4 max-w-lg space-y-4 animate-fade-in">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold">Notifications</h2>
                {notifications.length > 0 && (
                    <button onClick={onClearAll} className="text-sm font-semibold text-brand-neon hover:text-green-300 transition-colors">
                        Mark all as read
                    </button>
                )}
            </div>
                        ...
                        return (
                            <div key={notification.id} className={`bg-brand-secondary rounded-lg flex items-start space-x-4 transition-colors ${!notification.read ? 'border-l-4 border-brand-neon' : 'opacity-70'} p-4`}>
                                ...
                                <div className="flex-grow">
                                    <div
                                        className={`w-full text-left ${notification.postId ? 'cursor-pointer' : ''}`}
                                        onClick={() => notification.postId && onViewActivity(notification.postId)}
                                    >
