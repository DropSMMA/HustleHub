...
const PublicProfile: React.FC<PublicProfileProps> = ({
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 max-w-lg text-center">
        ...
        <button onClick={onBack} className="mt-4 text-brand-neon">
          Go Back
        </button>
      </div>
    );
  ...
  const renderConnectButton = () => {
    ...
    if (isConnected) {
      return (
        <button
          className="mt-4 bg-brand-tertiary text-brand-neon font-bold py-2 px-6 rounded-lg text-sm cursor-default"
          disabled
        >
          Connected
        </button>
    ...
    if (isPending) {
      return (
        <button
          className="mt-4 bg-brand-tertiary text-gray-400 font-bold py-2 px-6 rounded-lg text-sm cursor-wait"
          disabled
        >
          Pending...
        </button>
  ...
  return (
    <div className="container mx-auto px-4 max-w-lg space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <button
          onClick={() => onViewConnections(user.username)}
          className="w-full text-left"
        >
          <StatCard
            value={String(user.connections.length)}
            label="Connections"
          />
        </button>
      </div>
      ...
      {projects.length > 0 && (
        <div className="bg-brand-secondary rounded-xl p-4 space-y-2">
                ...
                {project.url && (
                  <>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-brand-neon hover:text-green-300 transition-colors"
                    >
                      Visit
                    </a>
                  </>
